    ...
    <div className="space-y-4">
      ...
      {useVariants && quickFill && (
        <div className="space-y-5">
          <div>
            ...
            <div className="p-3 border border-black/8 bg-[#F7F7F7] space-y-2">
              ...
              {quickColors.length > 0 && (
                <p className="text-[8px] text-black/40 font-serif italic">{quickColors.join(", ")}</p>
              )}
            </div>
          </div>
                ...
                {sizeOptions.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 text-[9px] uppercase tracking-widest border transition-all font-serif ${isSelected ? "bg-black text-white border-black" : "bg-white text-black/40 border-black/10 hover:border-black/30"}`}
                    >
                      {s}
                    </button>
                  );
          ...
          {quickSizes.length > 0 && (
            <div>
              <p className="text-[8px] uppercase tracking-[0.3em] text-black/40 mb-2 font-serif">Quantité des tailles</p>
              <div className="border border-black/8 divide-y divide-black/5">
                {quickSizes.map((s) => {
...
export default function ProductModel({
  ...
  const blurValidate = useCallback((field: keyof FieldErrors) => {
    ...
    if (fieldErrors[field]) {
      setErrors((e) => {
        if (e[field] === fieldErrors[field]) return e; // évite re-render si valeur identique
            ...
            return (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 text-[8px] uppercase tracking-[0.2em] font-serif transition-all border-b-2 ${
                  ...
                  : isDone  ? "border-transparent text-black/40 hover:text-black/70"
                           : "border-transparent text-black/30 hover:text-black/60"
                }`}
              >
                {isDone ? <Check size={10} strokeWidth={2.5} className="text-black/40" /> : <Icon size={11} strokeWidth={1.5} />}
                <span>{label}</span>
              </button>
            );
          })}
        ...
        >
          {step === 0 && (
            <StepBasicInfo
              ...
              setQuickSizes={setQuickSizes}
              setQuickQty={setQuickQty}
              setQuickColors={setQuickColors}
              setErrors={setErrors}
            />
          )}
          {step === 1 && (
