...
import { X, Plus, AlertCircle, ChevronDown, ChevronsRight, Zap } from "lucide-react";
...
export default function VariantBuilder({ variants, sizeOptions, onChange }: Props) {
  ...
  // ── État remplissage automatique ──────────────────────────────────────────
  const [autoOpen, setAutoOpen]           = useState(false);
  // size → stock par défaut pour l'auto-fill
  const [autoStocks, setAutoStocks]       = useState<Record<string, number>>({});
  // stock global pour remplir toutes les tailles d'un coup
  const [bulkStock, setBulkStock]         = useState<number>(1);
  // flash visuel après application
  ...
  /** Ajoute / retire une taille de la config auto */
  ...
  /** Met à jour le stock d'une taille spécifique dans la config auto */
  const updateAutoStock = (size: string, stock: number) =>
    setAutoStocks((prev) => ({ ...prev, [size]: Math.max(0, stock) }));

  /** Applique `bulkStock` à toutes les tailles déjà sélectionnées */
  const applyBulkToAll = () =>
    setAutoStocks((prev) =>
      Object.fromEntries(Object.keys(prev).map((k) => [k, bulkStock])),
    );

  /** Pousse la config auto vers toutes les variantes existantes */
  const applyAuto = () => {
    if (!autoSelected.length || !variants.length) return;
    onChange(
      variants.map((v) => ({
        ...v,
        sizes: autoSelected.map((name) => ({
          name,
          stock: autoStocks[name] ?? 1,
        })),
      })),
    );
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ══ PANNEAU REMPLISSAGE AUTOMATIQUE ════════════════════════════════ */}
      <div className={`border transition-colors ${
        applied
          ? "border-black/30 bg-black/3"
          : autoOpen
        ...
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-3 hover:bg-black/2 transition-colors text-left"
        >
        ...
        {/* Corps du panneau */}
        {autoOpen && (
          <div className="px-4 pb-4 pt-1 space-y-4 border-t border-[rgba(0,0,0,0.07)]">

            {/* ── Étape 1 : sélection des tailles ────────────────────────── */}
            <div>
            ...
            {/* ── Étape 2 : stock par taille (visible si tailles sélectionnées) */}
            {autoSelected.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {/* Raccourci "Tout mettre à N" */}
                  <div className="flex items-center gap-1.5">
                    <span className="text-[7px] text-black/20 font-serif">Tout à</span>
                    <div className="flex items-center border border-[rgba(0,0,0,0.1)]">
                      <button
                        type="button"
                        onClick={() => setBulkStock((n) => Math.max(0, n - 1))}
                        className="w-5 h-5 flex items-center justify-center text-black/30 hover:text-black transition-colors text-xs"
                      >
                        −
                      </button>
                      <input
                        ...
                        className="w-8 text-center text-[9px] font-serif border-x border-[rgba(0,0,0,0.1)] h-5 focus:outline-none bg-white"
                      />
                      <button
                        type="button"
                        onClick={() => setBulkStock((n) => n + 1)}
                        className="w-5 h-5 flex items-center justify-center text-black/30 hover:text-black transition-colors text-xs"
                      >
                        +
                      </button>
                ...
                {/* Stocks individuels par taille */}
                <div className="grid grid-cols-2 gap-1.5">
                  {autoSelected.map((s) => (
                    <div
                      key={s}
                      className="flex items-center border border-[rgba(0,0,0,0.08)] bg-white"
                    >
                      <span className="text-[9px] uppercase tracking-widest text-black/50 font-serif px-2.5 w-10 shrink-0">
                        {s}
                      </span>
                      <div className="flex items-center flex-1 border-l border-[rgba(0,0,0,0.08)]">
                        <button
                          type="button"
                          onClick={() => updateAutoStock(s, (autoStocks[s] ?? 1) - 1)}
                        ...
                        <input
                          type="number"
                          min={0}
                          value={autoStocks[s] ?? 1}
                          onChange={(e) => updateAutoStock(s, Number(e.target.value))}
                          className="flex-1 text-center text-[10px] font-serif border-x border-[rgba(0,0,0,0.08)] h-7 focus:outline-none bg-white"
                        />
                        <button
                          type="button"
                          onClick={() => updateAutoStock(s, (autoStocks[s] ?? 1) + 1)}
        ...
        return (
          <div
            ...
            className={`border bg-[#FAFAFA] p-4 space-y-4 transition-colors ${
              isDuplicate ? "border-red-300 bg-red-50/20" : "border-[rgba(0,0,0,0.1)]"
            }`}
          >
            {/* ── En-tête couleur ─────────────────────────────────────────── */}
            <div className="flex items-start gap-3">
                ...
                <input
                  ...
                  className={`w-full border text-[11px] py-2 px-3 focus:outline-none bg-white font-serif transition-colors ${
                    isDuplicate
                      ...
                      : "border-[rgba(0,0,0,0.12)] focus:border-black"
                  }`}
                />
                      ...
                      <button
                        ...
                        className={`w-5 h-5 rounded-full border-2 transition-all ${
                          isSelected
                            ...
                            : isUsed
                            ? "border-[rgba(0,0,0,0.08)] opacity-25 hover:opacity-50 cursor-not-allowed"
              ...
              <button
                ...
                className="mt-6 text-black/20 hover:text-red-500 transition-colors shrink-0"
              >
                ...
                {variant.sizes.map(({ name, stock, price }) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updatePrice(idx, name, e.target.value === "" ? undefined : Number(e.target.value))
                      }
                      placeholder="Base"
